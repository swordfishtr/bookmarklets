javascript:void async function(){try{
	// Header row
	...
	for (const p of roster) {
		if (
			...
			await addv(p,true,31,252,2);
			await addv(p,null,31,252,2);
		}
		await addv(p,true,31,252,1);
		await addv(p,null,31,252,1);
		await addv(p,true,31,252,0);
		await addv(p,null,31,252,0);
		await addv(p,null,31,0,0);
		if (p.baseStats.spe<=60 && Math.min(...Object.values(p.baseStats))===p.baseStats.spe) {
			await addv(p,false,0,0,0);
		}
	}
	vs.sort((a,b)=>a[0]-b[0]||a[8]-b[8]).reverse();
	for (let i = 600; i >= 0; i -= 100) {
		let msg = `between ${i} and ${i+100}`;
		if (i===600) msg = 'above 600';
		if (i===0) msg = 'below 100';
		msg = `Generated table for speed tiers ${msg} and copied to your clipboard! Paste directly into a google doc.`;
	...
	alert('All done! Have a good day now :3');
}catch(e){alert(e)}}();
